...
COST_LIBRARY = {
    "Direct Material": [
        "Raw Materials", "Basic Materials", "Components", "Sub-assemblies",
...
COST_LIBRARY["Inventory & Stock Adjustments"] = [
...
<style>
    ...
    div[data-testid="stMetric"] {
        border: 1px solid #D9E2EC; border-radius: 14px; padding: 12px;
        background: white;
    }
...
with st.sidebar:
    ...
    carriage_inward = st.number_input("Carriage / Freight Inward", min_value=0.0, value=0.0, step=100.0)
    ...
    profit_pct = st.number_input("Target Profit % on Cost", min_value=0.0, value=15.0, step=0.5)
    tax_pct = st.number_input("GST / Tax %", min_value=0.0, value=18.0, step=0.5)
...
st.markdown(
    '<div class="info-box"><b>Select every cost individually.</b> Choose a category, then choose the exact standard cost component. '
    'You can add the same component multiple times and use <b>Custom / Other</b> for anything not in the library.</div>',
    unsafe_allow_html=True
)

add_cols = st.columns([1.25, 2.15, 0.95, 1.05, 1.05])
with add_cols[0]:
...
with add_cols[2]:
    add_qty = st.number_input("Qty / Units", min_value=0.0, value=1.0, step=1.0, key="add_qty")
with add_cols[3]:
    add_rate = st.number_input("Rate", min_value=0.0, value=0.0, step=0.01, key="add_rate")
with add_cols[4]:
    st.write("")
    st.write("")
...
with st.expander("✏️ Add a Custom Cost Component", expanded=False):
    cc1, cc2, cc3, cc4 = st.columns([1.25, 2.15, 0.95, 1.05])
    with cc1:
    ...
    with cc3:
        custom_qty = st.number_input("Qty / Units", min_value=0.0, value=1.0, step=1.0, key="custom_qty")
    with cc4:
        custom_rate = st.number_input("Rate", min_value=0.0, value=0.0, step=0.01, key="custom_rate")
...
if not st.session_state.cost_lines:
    ...
    display_df["Amount"] = display_df["Qty / Units"] * display_df["Rate"]
    ...
    remove_options = [
        f"{i+1}. {row['Category']} — {row['Cost Component']} — {currency}{row['Qty / Units']*row['Rate']:,.2f}"
...
if df.empty:
    df = pd.DataFrame(columns=["Category", "Cost Component", "Qty / Units", "Rate"])
df["Amount"] = pd.to_numeric(df.get("Qty / Units", pd.Series(dtype=float)), errors="coerce").fillna(0) * \
               pd.to_numeric(df.get("Rate", pd.Series(dtype=float)), errors="coerce").fillna(0)

def cat_total(category):
    return float(df.loc[df["Category"] == category, "Amount"].sum())

# Inventory values are available in TWO ways:
# 1) Dedicated setup controls in the sidebar, and
# 2) Selectable line items in the main "Cost Component" selector.
# If inventory lines are entered in the main selector, they override the
# corresponding sidebar values so the user can build the complete sheet
# entirely from selectable components.
def selected_inventory_value(component, default):
    rows = df.loc[
        (df["Category"] == "Inventory & Stock Adjustments") &
        (df["Cost Component"] == component),
        "Amount"
    ]
    return float(rows.sum()) if not rows.empty else float(default)

opening_rm_calc = selected_inventory_value("Opening Raw Material Stock", opening_rm)
closing_rm_calc = selected_inventory_value("Closing Raw Material Stock", closing_rm)
purchases_rm_calc = selected_inventory_value("Raw Material Purchases", purchases_rm)
carriage_inward_calc = selected_inventory_value("Carriage / Freight Inward", carriage_inward)
opening_wip_calc = selected_inventory_value("Opening Work-in-Progress (WIP)", opening_wip)
closing_wip_calc = selected_inventory_value("Closing Work-in-Progress (WIP)", closing_wip)
opening_fg_calc = selected_inventory_value("Opening Finished Goods", opening_fg)
closing_fg_calc = selected_inventory_value("Closing Finished Goods", closing_fg)

# Raw-material flow:
# Opening RM + Purchases + Carriage Inward - Closing RM = Material Consumed.
material_available = opening_rm_calc + purchases_rm_calc + carriage_inward_calc
material_consumed = material_available - closing_rm_calc

non_raw_material_direct = float(
    df.loc[
        (df["Category"] == "Direct Material"),
        "Amount"
    ].sum()
)
material = material_consumed + non_raw_material_direct
...
tax = sales_value_before_tax * tax_pct / 100.0
final_invoice_value = sales_value_before_tax + tax
...
metrics = [
    ...
    ("Profit", profit),
    ("Final Invoice", final_invoice_value),
]
...
u[3].metric("GST / Tax", f"{currency}{tax:,.2f}")

# =========================
# STANDARD COST SHEET
# =========================
st.markdown('<div class="section-title">4. Standard Cost Sheet</div>', unsafe_allow_html=True)

summary_rows = [
    ("OPENING RAW MATERIAL STOCK", opening_rm_calc, "adjustment"),
    ("ADD: RAW MATERIAL PURCHASES", purchases_rm_calc, "adjustment"),
    ("ADD: CARRIAGE / FREIGHT INWARD", carriage_inward_calc, "adjustment"),
    ("LESS: CLOSING RAW MATERIAL STOCK", -closing_rm_calc, "adjustment"),
    ("RAW MATERIAL CONSUMED", material_consumed, "total"),
    ("OTHER DIRECT MATERIAL", non_raw_material_direct, "section"),
    ...
    ("SALES VALUE BEFORE TAX", sales_value_before_tax, "grand"),
    ("ADD: GST / TAX", tax, "tax"),
    ("FINAL INVOICE VALUE", final_invoice_value, "grand"),
]
...
with tab2:
    if not df.empty:
        detail = df[["Category", "Cost Component", "Qty / Units", "Rate", "Amount"]].copy()
        st.dataframe(
            detail.style.format({
                "Qty / Units": "{:,.2f}",
                "Rate": f"{currency}{{:,.2f}}",
...
1. **Raw Material Consumed** = Opening Raw Material Stock + Raw Material Purchases + Carriage/Freight Inward − Closing Raw Material Stock. These four items can now be selected individually from **Inventory & Stock Adjustments**.  
...
6. **Profit** = Cost of Sales × Target Profit % on Cost  
7. **Sales Value Before Tax** = Cost of Sales + Profit  
8. **GST / Tax** = Sales Value Before Tax × Tax %  
9. **Final Invoice Value** = Sales Value Before Tax + GST / Tax

> The model treats opening/closing WIP and finished-goods figures as **cost-value adjustments**, consistent with a standard cost-sheet presentation. Tax is calculated separately on the pre-tax sales value.
""")
...
def build_excel():
    ...
    for category in CATEGORIES:
        ...
        for _, item in subset.iterrows():
            ...
            ws.cell(r, 4, float(item["Rate"]))
            ws.cell(r, 5, float(item["Amount"]))
    ...
    for label, val, kind in summary_rows:
        ...
        ws.cell(r, 6, f'=IFERROR(E{r}/E{summary_excel_rows.get("SALES VALUE BEFORE TAX", r)},0)')
    ...
    sales_row = summary_excel_rows["SALES VALUE BEFORE TAX"]
    ...
    unit_rows = [
        ...
        ("Carriage / Freight Inward", carriage_inward, False),
        ...
        ("Selling Price / Unit", unit_price, False),
        ("Profit / Unit", unit_profit, False),
        ("Target Profit % on Cost", profit_pct, True),
        ("GST / Tax %", tax_pct, True),
    ]
    ...
    detail_headers = ["Category", "Cost Component", "Qty / Units", "Rate", "Amount"]
    for c, header in enumerate(detail_headers, 1):
        cell = detail_ws.cell(1, c, header)
        cell.font = Font(bold=True, color=WHITE)
        cell.fill = PatternFill("solid", fgColor=TEAL)
        cell.alignment = Alignment(horizontal="center")
    for i, width in enumerate([28, 48, 16, 16, 20], 1):
    ...
    input_rows = [
        ...
        ("Carriage / Freight Inward", carriage_inward),
        ...
        ("Target Profit % on Cost", profit_pct / 100),
        ("GST / Tax %", tax_pct / 100),
    ]
