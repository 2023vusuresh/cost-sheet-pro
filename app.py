...
# Direct Material inventory/purchases.
opening_rm_calc = selected_component_value(
    "Direct Material", "Opening Raw Material Stock", opening_rm
)
purchases_rm_calc = selected_component_value(
    "Direct Material", "Raw Material Purchases", purchases_rm
)
closing_rm_calc = selected_component_value(
    "Direct Material", "Closing Raw Material Stock", closing_rm
)

# WIP and finished goods adjustments.
opening_wip_calc = selected_component_value(
    "Cost Sheet Adjustments", "Opening Work-in-Progress (WIP)", opening_wip
)
closing_wip_calc = selected_component_value(
    "Cost Sheet Adjustments", "Closing Work-in-Progress (WIP)", closing_wip
)
opening_fg_calc = selected_component_value(
    "Cost Sheet Adjustments", "Opening Finished Goods", opening_fg
)
closing_fg_calc = selected_component_value(
    "Cost Sheet Adjustments", "Closing Finished Goods", closing_fg
)

# Other Direct Material excludes the three stock/purchase lines.
...
# Profit calculation is user-selected.
if profit_basis == "Profit % on Cost":
    profit = cost_of_sales * profit_pct / 100.0
    sales_value_before_tax = cost_of_sales + profit
else:
    # Prevent division by zero and keep the input capped below 100%.
    sales_value_before_tax = cost_of_sales / (1.0 - profit_pct / 100.0)
    profit = sales_value_before_tax - cost_of_sales

# Tax is intentionally excluded from the cost sheet and Excel output.
tax = 0.0
final_invoice_value = sales_value_before_tax

# Per-unit figures.
cost_per_unit = cost_of_sales / units if units > 0 else 0.0
selling_price_per_unit = sales_value_before_tax / units if units > 0 else 0.0
profit_per_unit = profit / units if units > 0 else 0.0
sales_value_per_unit = selling_price_per_unit
