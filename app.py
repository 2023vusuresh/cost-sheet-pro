...
direct_material_costs = cat_total("Direct Material")
material_available = opening_rm + purchases_rm + carriage_inward
material_consumed = material_available - closing_rm

# If individual Direct Material components are entered, they are added to the
# inventory-derived material consumption only when they are not the main
# "Raw Materials" line. This avoids double counting the standard Raw Materials
# input while still allowing components such as direct packing/chemicals.
non_raw_material_direct = float(
    df.loc[
        (df["Category"] == "Direct Material") &
        (df["Cost Component"] != "Raw Materials"),
        "Amount"
    ].sum()
)
...
works_cost = prime_cost + factory_oh + opening_wip - closing_wip
cost_of_production = works_cost + admin_oh
cost_of_goods_sold = cost_of_production + opening_fg - closing_fg
...
summary_rows = [
    ("OPENING RAW MATERIAL STOCK", opening_rm, "adjustment"),
    ("ADD: RAW MATERIAL PURCHASES", purchases_rm, "adjustment"),
    ("ADD: CARRIAGE / FREIGHT INWARD", carriage_inward, "adjustment"),
    ("LESS: CLOSING RAW MATERIAL STOCK", -closing_rm, "adjustment"),
    ...
    ("ADD: OPENING WIP", opening_wip, "adjustment"),
    ("LESS: CLOSING WIP", -closing_wip, "adjustment"),
    ("WORKS / FACTORY COST", works_cost, "total"),
    ("ADMINISTRATION OVERHEAD", admin_oh, "section"),
    ("COST OF PRODUCTION", cost_of_production, "total"),
    ("ADD: OPENING FINISHED GOODS", opening_fg, "adjustment"),
    ("LESS: CLOSING FINISHED GOODS", -closing_fg, "adjustment"),
...
1. **Raw Material Consumed** = Opening Raw Material Stock + Raw Material Purchases + Carriage/Freight Inward − Closing Raw Material Stock  
