...
<style>
    ...
    div[data-testid="stVerticalBlockBorderWrapper"] {margin-bottom: 14px;}
    div[data-testid="stVerticalBlockBorderWrapper"] .stButton button {min-height: 44px; font-weight: 750;}
    div[data-testid="stVerticalBlockBorderWrapper"] [data-testid="stNumberInput"] input {font-variant-numeric: tabular-nums;}
    .input-card {
...
# Use two rows instead of one long row so labels, inputs and the
# Add button remain readable on normal laptop widths.
with st.container(border=True):
    st.markdown("**Cost component input**")
    row1 = st.columns([1.25, 2.15, 1.15], gap="medium")
    with row1[0]:
    ...
    row2 = st.columns([1.25, 1.35, 1.35, 1.35], gap="medium")
    with row2[0]:
    ...
    if add_basis == "Total Amount":
        ...
        with row2[2]:
            st.number_input(
                "Rate / Unit (Auto)",
                min_value=0.0,
                value=float(add_auto_rate),
                disabled=True,
                key="add_auto_rate_display",
                format="%.2f"
            )
    else:
        with row2[1]:
        ...
        with row2[2]:
            st.number_input(
                "Total Amount (Auto)",
                min_value=0.0,
                value=float(add_total),
                disabled=True,
                key="add_total_display",
                format="%.2f"
            )
...
with st.expander("✏️ Add a Custom Cost Component", expanded=False):
    st.markdown("**Custom component input**")
    crow1 = st.columns([1.25, 2.15, 1.15], gap="medium")
    with crow1[0]:
        custom_category = st.selectbox("Category", CATEGORIES, key="custom_category")
    with crow1[1]:
        custom_name = st.text_input(
            "Custom component name",
            placeholder="e.g., Special Compliance Fee"
        )
    with crow1[2]:
    ...
    crow2 = st.columns([1.25, 1.35, 1.35, 1.35], gap="medium")
    with crow2[0]:
    ...
    if custom_basis == "Total Amount":
        ...
        with crow2[2]:
            st.number_input(
                "Rate / Unit (Auto)",
                min_value=0.0,
                value=float(custom_auto_rate),
                disabled=True,
                key="custom_auto_rate_display",
                format="%.2f"
            )
    else:
        with crow2[1]:
