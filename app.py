...
with st.sidebar:
    ...
    profit_basis = st.selectbox(
        ...
        index=1,
        help="Choose whether the profit percentage is calculated on Cost of Sales or on Sales."
    )
    profit_pct = st.number_input(
        ...
        value=20.0,
        step=0.5,
        help="Enter the target profit percentage according to the selected basis."
    )
...
opening_rm_calc = selected_component_value("Direct Material", "Opening Raw Material Stock", 0.0)
purchases_rm_calc = selected_component_value("Direct Material", "Raw Material Purchases", 0.0)
closing_rm_calc = selected_component_value("Direct Material", "Closing Raw Material Stock", 0.0)

# WIP and finished-goods adjustments are selectable under Cost Sheet Adjustments.
opening_wip_calc = selected_component_value("Cost Sheet Adjustments", "Opening Work-in-Progress (WIP)", 0.0)
closing_wip_calc = selected_component_value("Cost Sheet Adjustments", "Closing Work-in-Progress (WIP)", 0.0)
opening_fg_calc = selected_component_value("Cost Sheet Adjustments", "Opening Finished Goods", 0.0)
closing_fg_calc = selected_component_value("Cost Sheet Adjustments", "Closing Finished Goods", 0.0)
...
# Profit is explicitly calculated using the user's selected basis.
...
# GST/Tax is intentionally excluded from the standard cost sheet and Excel.
tax = 0.0
...
st.dataframe(
    ...
    use_container_width=True,
    hide_index=True,
    height=min(620, max(300, 58 + len(summary_df) * 35)),
)
...
**Raw Material Consumed** = Opening Raw Material Stock + Raw Material Purchases − Closing Raw Material Stock

**Prime Cost** = Direct Material + Direct Labour + Direct Expenses

**Works / Factory Cost** = Prime Cost + Factory Overhead + Opening WIP − Closing WIP

**Cost of Production** = Works / Factory Cost + Administration Overhead
...
**Profit % on Cost:** Profit = Cost of Sales × Profit %

**Profit % on Sales:** Sales = Cost of Sales ÷ (1 − Profit %); Profit = Sales − Cost of Sales

The selected profit basis is recorded in the Excel output. GST/Tax is excluded from the cost-sheet calculation and Excel output.
""")
...
def build_excel():
    ...
    for category in CATEGORIES:
        ...
        for _, item in subset.iterrows():
            ...
            # Percentage formula is filled after the Sales Value row is known.
            ws.cell(r, 6, 0)
            for c in range(1, 7):
    ...
    for label, val, kind in summary_rows:
        ...
        ws.cell(r, 6, f'=IFERROR(E{r}/E{summary_excel_rows.get("SALES VALUE", r)},0)')
    ...
    # Also calculate component-level percentage of Sales without referencing
    # a row that has not yet been created.
    for row_num in range(6, summary_start):
        if ws.cell(row_num, 2).value and ws.cell(row_num, 2).value not in [
            "DIRECT MATERIAL", "DIRECT LABOUR", "DIRECT EXPENSES",
            "FACTORY OVERHEAD", "ADMINISTRATION OVERHEAD",
            "SELLING & DISTRIBUTION", "COST SHEET ADJUSTMENTS"
        ]:
            ws.cell(row_num, 6, f'=IFERROR(E{row_num}/E{sales_row},0)')
            ws.cell(row_num, 6).number_format = '0.0%'
    ...
    input_rows = [
        ...
        ("Opening Raw Material Stock", opening_rm_calc),
        ("Raw Material Purchases", purchases_rm_calc),
        ("Closing Raw Material Stock", closing_rm_calc),
        ("Opening WIP", opening_wip_calc),
        ("Closing WIP", closing_wip_calc),
        ("Opening Finished Goods", opening_fg_calc),
        ("Closing Finished Goods", closing_fg_calc),
