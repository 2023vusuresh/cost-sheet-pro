...
add_cols = st.columns([1.05, 1.90, 0.85, 1.10, 1.15, 1.10, 0.95], gap="medium")
with add_cols[0]:
...
with add_cols[3]:
    add_basis = st.selectbox(
        ...
        key="add_basis",
        help="Use Total Amount when the question gives a total cost. Use Rate / Unit when the question gives a per-unit rate."
    )
with add_cols[4]:
    if add_basis == "Total Amount":
        add_entered = st.number_input("Total Amount", min_value=0.0, value=0.0, step=100.0, key="add_total")
        ...
        add_entered = st.number_input("Rate / Unit", min_value=0.0, value=0.0, step=1.0, key="add_rate")
        add_auto_rate = float(add_entered)
        add_total = add_qty * add_auto_rate
        st.number_input("Total Amount", min_value=0.0, value=float(add_total), disabled=True, key="add_total_display", format="%.2f")
with add_cols[5]:
    st.metric("Calculated", f"{currency}{add_total:,.2f}")
with add_cols[6]:
    st.write("")
    st.write("")
...
with st.expander("✏️ Add a Custom Cost Component", expanded=False):
    cc1, cc2, cc3, cc4, cc5, cc6 = st.columns([1.00, 1.90, 0.85, 1.05, 1.15, 1.05], gap="medium")
    with cc1:
    ...
    with cc5:
        if custom_basis == "Total Amount":
            ...
            st.number_input("Total Amount", min_value=0.0, value=float(custom_total), disabled=True, key="custom_total_display", format="%.2f")
    with cc6:
