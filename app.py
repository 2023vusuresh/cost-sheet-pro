...
<style>
    ...
    .hero {
        ...
        background: linear-gradient(135deg, #0B2545 0%, #1D5D85 55%, #2C7DA0 100%);
        color: white; box-shadow: 0 12px 34px rgba(11,37,69,.16);
    }
    .hero h1 {font-size: clamp(2rem, 3.2vw, 2.7rem); margin: 0; font-weight: 850; letter-spacing: -.02em;}
    .hero p {margin: 8px 0 0; color: #E7F2FA; font-size: 1rem;}
    .section-title {
        font-size: 1.25rem; font-weight: 800; color: #102A43;
        margin: 26px 0 10px; padding-bottom: 6px;
    }
    .info-box {
        border: 1px solid #D9E2EC; border-radius: 15px; padding: 13px 16px;
        background: linear-gradient(180deg,#FBFDFF 0%,#F7FAFC 100%); margin-bottom: 14px;
        box-shadow: 0 3px 12px rgba(16,42,67,.04);
    }
    .small-note {color:#52606D; font-size:.86rem;}
    .input-card {
        border: 1px solid #D9E2EC; border-radius: 17px; padding: 18px 18px 10px;
        background: #FFFFFF; box-shadow: 0 5px 18px rgba(16,42,67,.06); margin-bottom: 12px;
    }
    div[data-testid="stMetric"] {
        border: 1px solid #D9E2EC; border-radius: 16px; padding: 16px 17px;
        background: #FFFFFF; min-height: 104px; box-shadow: 0 4px 14px rgba(16,42,67,.05);
    }
    div[data-testid="stMetricLabel"] {white-space: normal !important; font-weight: 650;}
    div[data-testid="stMetricValue"] {
        font-size: clamp(1.25rem, 2.0vw, 1.85rem) !important;
        white-space: nowrap !important; overflow: hidden !important; text-overflow: ellipsis !important;
        line-height: 1.2 !important;
    }
    div[data-testid="stVerticalBlock"] > div:has(> div.stButton) button {border-radius: 11px;}
    button[kind="primary"] {font-weight: 750; border-radius: 11px;}
    [data-testid="stSidebar"] {border-right: 1px solid #E4EAF0;}
...
<div class="hero">
  ...
  <p>Build a standard cost sheet from mixed question formats • automatic unit rates • live costing • professional Excel export</p>
</div>
""", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    st.caption("Set the basic details first, then build your cost components.")
...
st.markdown(
    '<div class="info-box"><b>Enter the cost exactly as it appears in the question.</b> Choose a category and component, enter the quantity/units, then select <b>Total Amount</b> or <b>Rate / Unit</b>. The other value is calculated automatically. No manual conversion is required.</div>',
    unsafe_allow_html=True
)

# Spacious five-column layout: preserve the original look while keeping
# the value-entry controls on their own line so nothing overlaps.
with st.container(border=True):
    top = st.columns([1.10, 2.15, 1.00, 1.15], gap="medium")
    with top[0]:
    ...
    with top[2]:
        add_qty = st.number_input(
            "Total Qty / Units", min_value=0.0, value=1.0,
    ...
    with top[3]:
        add_basis = st.selectbox(
            ...
            key="add_basis",
            help="Choose the format used in the question. The other value is calculated automatically."
        )

    bottom = st.columns([1.35, 1.35, 1.35, 1.05], gap="medium")
    if add_basis == "Total Amount":
        with bottom[0]:
            add_entered = st.number_input(
                "Total Amount", min_value=0.0, value=0.0,
                step=100.0, key="add_total"
            )
        add_total = float(add_entered)
        add_auto_rate = add_total / add_qty if add_qty else 0.0
        with bottom[1]:
            st.number_input(
                "Rate / Unit (Auto)", min_value=0.0,
                value=float(add_auto_rate), disabled=True,
                key="add_auto_rate_display", format="%.2f"
            )
    else:
        with bottom[0]:
            add_entered = st.number_input(
                "Rate / Unit", min_value=0.0, value=0.0,
                step=1.0, key="add_rate"
            )
        add_auto_rate = float(add_entered)
        add_total = add_qty * add_auto_rate
        with bottom[1]:
            st.number_input(
                "Total Amount (Auto)", min_value=0.0,
                value=float(add_total), disabled=True,
                key="add_total_display", format="%.2f"
            )

    with bottom[2]:
        st.markdown('<div class="small-note" style="padding-top:30px;">The total amount and rate are kept mathematically consistent.</div>', unsafe_allow_html=True)
    with bottom[3]:
        st.write("")
        if st.button("➕ Add Cost", type="primary", use_container_width=True, key="add_cost_button"):
            if add_qty <= 0:
...
with st.expander("✏️ Add a Custom Cost Component", expanded=False):
    ...
    with top[3]:
        custom_basis = st.selectbox("Enter By", ["Total Amount", "Rate / Unit"], key="custom_basis")

    bottom = st.columns([1.35, 1.35, 1.35, 1.05], gap="medium")
    if custom_basis == "Total Amount":
        with bottom[0]:
            custom_entered = st.number_input("Total Amount", min_value=0.0, value=0.0, step=100.0, key="custom_total")
        custom_total = float(custom_entered)
        custom_auto_rate = custom_total / custom_qty if custom_qty else 0.0
        with bottom[1]:
            st.number_input("Rate / Unit (Auto)", min_value=0.0, value=float(custom_auto_rate), disabled=True, key="custom_auto_rate_display", format="%.2f")
    else:
        with bottom[0]:
            custom_entered = st.number_input("Rate / Unit", min_value=0.0, value=0.0, step=1.0, key="custom_rate")
        custom_auto_rate = float(custom_entered)
        custom_total = custom_qty * custom_auto_rate
        with bottom[1]:
            st.number_input("Total Amount (Auto)", min_value=0.0, value=float(custom_total), disabled=True, key="custom_total_display", format="%.2f")
    with bottom[2]:
        st.markdown('<div class="small-note" style="padding-top:30px;">The total amount and rate are kept mathematically consistent.</div>', unsafe_allow_html=True)
    with bottom[3]:
        if st.button("Add Custom Cost", use_container_width=True, key="add_custom_cost_button"):
...
if not st.session_state.cost_lines:
    st.info("👆 No cost components added yet. Start above by selecting a category, choosing the component, and entering the amount or rate from your question.")
else:
...
st.markdown(f'<div class="info-box"><b>Live result:</b> {units:,.0f} units • Profit basis: <b>{profit_pct:.2f}% {"on Sales" if profit_basis == "Profit % on Sales" else "on Cost"}</b></div>', unsafe_allow_html=True)
m1 = st.columns(3, gap="medium")
...
def build_excel():
    ...
    unit_rows = [
        ...
        ("Opening Raw Material Stock", opening_rm_calc, False),
        ("Raw Material Purchases", purchases_rm_calc, False),
        ("Closing Raw Material Stock", closing_rm_calc, False),
        ("Opening WIP", opening_wip_calc, False),
        ("Closing WIP", closing_wip_calc, False),
        ("Opening Finished Goods", opening_fg_calc, False),
        ("Closing Finished Goods", closing_fg_calc, False),
...
st.markdown(
    '<div class="info-box">Your Excel workbook contains a formatted <b>Cost Sheet</b> and '
    '<b>Cost Components</b> detail sheet.</div>',
    unsafe_allow_html=True
)
