...
with st.sidebar:
    ...
    profit_pct = st.number_input(
        "Enter Profit Margin (%)", min_value=0.0, max_value=99.99,
        value=20.0, step=0.5, format="%.2f",
        help="Example: 20 means 20%."
    )
    st.caption(f"Selected: {profit_pct:.2f}% {'of Cost of Sales' if profit_basis == 'Profit % on Cost' else 'of Sales'}")
...
st.success(f"Profit basis: {profit_pct:.2f}% {'on Cost of Sales' if profit_basis == 'Profit % on Cost' else 'on Sales'}")
